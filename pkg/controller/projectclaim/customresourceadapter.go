...
func (c *CustomResourceAdapter) EnsureProjectClaimState(state) error {
	if c.projectClaim.Status.State == state {
		return nil
	}

	if state == gcpv1alphav1.ClaimStatusPending {
		if c.projectClaim.Status.State != "" {
			return nil
		}
	} 

	if state == gcpv1alphav1.ClaimStatusPendingProject {
	...
	err = c.client.Status().Update(context.TODO(), projectClaim)
	if err != nil {
